...
\begin{tabular}{| l | r |}
    ...
    \texttt{context.getImageData()} & {Returns an object containing data about the canvas element inluding a full pixel map} \\ \hline
    \texttt{canvas.toDataUrl()} & {Returns a URL which contains the canvas itself (fully rendered)} \\
    \hline
\end{tabular}
\caption{Table explaining some HTML \texttt{<canvas>} functions}
