...
\begin{minipage}{\linewidth}
\begin{lstlisting}[caption={Injecting a script into the web page}, label={lst:injection}]
...
Listing~\ref{lst:create-element} shows how this is achieved without breaking the function entirely.
...
One case that this doesn't solve for as mentioned before is for when there's a canvas within an \texttt{<iframe>}.
This is because the \texttt{MutationObserver} can't access the contents of an iframe that's added.
For this reason, the app has to find iframes in the same way that it scans for \texttt{<canvas>} elements, but then has to do some processing after it's gathered them.
...
If the iframe is loaded in from a different domain than the one that owns the parent webpage, then the parent webpage (and by consequence the script that's injected by the add-on) can not access the contents of the iframe.
...
After visiting a few websites it was clear that this wasn't a very good solution and isn't feasible as a long-term method of defence.
\url{panopticlick.eff.org}, another online fingerprint checker can't successfully take a fingerprint when the add-on is activated with the iframe deletion code in effect.
Although it may be seen as a positive thing that a fingerprinter doesn't work, this is not the case for other comparable fingerprinting websites.
Many other sites simply don't function with the deletion of all of the iframe content on their pages.
